...
而是一个更深刻的\textbf{目标冲突}：
当我们要求模型"遵循人类指令"（Thread~1, \crossref{1}{sec:t1}）
并"最大化人类偏好"（Thread~2, \crossref{2}{sec:t2}）时，
...
与 mechanistic interpretability（\crossref{4}{sec:t4}）自底向上分析个别神经元和电路不同，
...
Zou et al.\ 展示了 RepE 可以有效控制模型的多种高层行为，
包括 honesty、utility、morality、power-seeking、
harmlessness、fairness 和 knowledge editing 等。
...
Thread~3 的演化呈现出清晰的主线：
从 HH-RLHF 暴露 helpfulness--harm\-less\-ness 张力，
...
这条线程与 Thread~2（preference optimization 提供基础框架，\crossref{2}{sec:t2}）、
Thread~4（reasoning 能力影响模型遵循安全推理的可靠性，\crossref{4}{sec:t4}）、
