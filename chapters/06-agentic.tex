...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Action}：执行具体的环境操作（如 \texttt{search[e]}、\texttt{lookup[s]}、\texttt{finish[a]}）。
...
% ============================================================
\subsection{收敛与前沿：Function Calling 与 Planning 的统一}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{Branch C} 的能力——生成语法正确且语义精确的代码 patch（code alignment，\crossref{1}{sec:t1-distillation}）；
...
这使得 RLVR 范式（\crossref{2}{sec:t2-rlvr}）可以直接应用于编码 agent 训练。

\citet{yang2024sweagent} 的 \textbf{SWE-Agent}
设计了 \textbf{Agent-Computer Interface (ACI)}——
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Agent SFT}：
    ...
    （类似 Thread~1 中的合成数据方法，\crossref{1}{sec:t1-synthetic}），
    以及产品部署中收集的真实用户交互轨迹。
    关键是轨迹的\emph{格式}——
...
\node[branch, below=1.2cm of prod] (conv) {收敛：Agent SFT + RL\\+ 环境优化 三阶段范式};
...
\end{tikzpicture}
\caption{Thread 6 演化路径：从 RL-based tool use 到 coding agent 生产级部署。
青色节点为 landmark 工作，灰色节点为重要分支。}
\label{fig:agentic-evolution}
\end{figure}

\begin{table}[!ht]
\centering
\caption{Thread 6 核心方法对比。Action Space 区分 API 调用、代码执行和 GUI 操作。}
...
\evolution{LLM 作为被动的文本生成器}{LLM 作为主动的环境交互者，Agent 进入生产级部署}{tool learning + ReAct + RL from environment}
