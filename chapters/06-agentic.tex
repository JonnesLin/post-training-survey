...
早期 function calling 训练只关注"给定工具列表，正确生成调用"，而忽略了多步骤规划能力。最新的 practice 将 ReAct/DFSDT 风格的推理轨迹与 function calling 数据融合训练，使模型同时具备"知道调什么"和"知道按什么顺序调"的能力。xLAM \citep{zhang2024xlam} 和 Granite Function Calling \citep{abdelaziz2024granite} 是这一趋势的代表。
...
也成为四条分支收敛的标志性案例。
\citet{jimenez2024swebench} 提出的 \textbf{SWE-bench}
将评估标准从人造编程题推进到真实的 GitHub issue resolution：
给定一个 repository 和 issue description，
agent 需要自主定位问题代码、设计修复方案并生成可通过原始 test suite 的 patch。
SWE-bench 的 2,294 个实例覆盖了 12 个大型 Python 项目，
平均每个 issue 涉及跨文件修改——
这同时要求：
\begin{itemize}[leftmargin=*]
  \item \emph{Branch A} 的能力——精确调用 bash、编辑器、搜索工具（function calling）；
  \item \emph{Branch B} 的能力——多步骤推理与规划，包括错误诊断、假设验证、方案回退（agent tuning）；
  \item \emph{Branch C} 的能力——生成语法正确且语义精确的代码 patch（code alignment）；
...
相同底层模型在不同 action interface 下的 resolve rate 差异可达两倍以上，
表明 agentic post-training 不仅要优化模型参数，
还需要精心设计模型与环境的交互协议。
...
提供了可扩展的 sandbox 执行环境和标准化的 agent 评估框架。
...
Cognition AI 于 2024 年初发布 \textbf{Devin} \citep{cognition2024devin}，
定位为``世界首个 AI 软件工程师''。
Devin 配备了完整的开发环境（终端、代码编辑器、浏览器），
能够自主执行多步骤的软件工程任务——
从阅读 issue、搜索代码库、编写代码到运行测试。
尽管业界对其实际能力存在争议，
Devin 的发布标志着 agentic capabilities 从学术研究走向产品化的转折点，
也催生了一轮编码 agent 的产品竞赛。
...
通过 MCP 协议集成外部工具，
Claude Code 展示了一种``agent as CLI tool''的产品范式，
强调与人类开发者的协作而非替代。
...
在 SWE-bench 上观察到的快速进步
是否遵循可预测的 scaling curve，
还是会在某个复杂度阈值处遭遇根本性瓶颈？
\end{openproblembox}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{能力收敛}（2024）：
    ...
    AgentBench 等系统化评估框架的出现标志着领域走向成熟。

  \item \textbf{产品化与基础设施}（2025--2026）：
    Computer use 将 action space 从离散 API 扩展到连续 GUI 操作，
    MCP 等标准协议和 Agent SDK 的出现降低了 agent 系统的构建门槛，
    Devin、Claude Code 等产品展示了 agent 的商业化路径。
\end{enumerate}
...
Thread~4 的推理能力为 agent 提供了规划层（ReAct 的 Thought 本质上就是 CoT）；
