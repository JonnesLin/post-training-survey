...
并与 Thread~6 中 tool-use 的思想形成呼应（\crossref{6}{sec:thread6}）。
...
\subsubsection{ReST \& ReST\textsuperscript{EM}: 强化自训练}
...
GRPO 的算法细节和统一视角已在 \S\ref{sec:pref:grpo} 中详细推导，
此处聚焦其对推理任务的特殊意义。
...
这一结论深刻影响了 Thread~2 的研究方向（\crossref{2}{sec:pref}）。
...
R1 & 4-stage: SFT$\to$RL$\to$RS$\to$RL & Rule-based + preference RM & 79.8\% AIME; distillable to small models \\
\bottomrule
\end{tabularx}
\end{table}
