...
\crossref{1}{sec:sft}
RLHF 的引入（\crossref{2}{sec:pref}）标志着 post-training 进入了优化人类偏好的阶段，
但早期 RLHF 仍高度依赖 SFT warm start，RL 更像是一个"微调的微调"。
真正的转折点出现在 reasoning 线程（\crossref{4}{sec:thread4}）：
...
（\crossref{7}{sec:efficiency:compute-rl}）进一步降低了 RL 训练的门槛，
...
（\crossref{7}{sec:efficiency:synthetic}），
...
在 verifiable reward 上的成功（\crossref{4}{sec:thread4}）
引发了一个根本性问题：
...
模型的目标行为从"回答单个问题"
演化为"在环境中完成多步骤复杂任务"。
早期 SFT 关注的是单轮 instruction following（\crossref{1}{sec:sft}），
随后扩展到多轮对话（LIMA 的 30 条对话示例的消融实验即证明了这一方向的重要性，
\crossref{7}{sec:efficiency:data-selection}），
再到 tool use（Toolformer \cite{schick2023toolformer}，\crossref{6}{sec:thread6}），
...
（\crossref{7}{sec:efficiency:foundation}），
...
SFT 不再是最终步骤而是推理能力冷启动的基础（\crossref{1}{sec:sft:2025}），
RLVR 成为获取推理能力的核心训练方法（\crossref{2}{sec:pref:rlvr}），
...
\label{sec:conclusion:meta}

在七条宏观趋势之下，
七条线程的演化过程中反复出现了若干结构性的 meta-pattern。
这些 pattern 不限于某一线程，
...
\paragraph{方向六：Adjustable Reasoning as a Service。}
趋势六中 thinking models 的普及引出了一个新的系统设计问题：
\textbf{如何让用户在推理深度与响应延迟之间动态权衡？}
Kimi k1.5 的 long2short 训练 \cite{moonshot2025kimi} 和
Qwen3 的 thinking mode fusion \cite{yang2025qwen3}
已经展示了在同一模型中实现可调推理预算的可行性。
这一问题的核心挑战已被实证清晰地定义：
Chen et al.\ \cite{chen2025underthinking} 系统地量化了
underthinking（过早终止推理，错失正确路径）
与 overthinking（冗余推理，浪费计算资源）两种失败模式的分布特征；
Sui et al.\ \cite{sui2025stopoverthinking} 的综述
将 efficient reasoning 的方法论系统化为
model-based（训练阶段优化模型简洁性）、
reasoning output-based（推理时动态缩减步骤）和
input prompt-based（通过 prompt 属性控制效率）三类策略。

\noindent
在 post-training 层面，adaptive compute allocation 正在从概念走向具体实现。
AdaptThink \cite{zhang2025adaptthink} 训练模型自主选择
Think（深度推理）或 NoThink（直接回答）模式——
通过 RL reward 强化模式选择的准确性，
使模型在简单问题上节省推理 token 而在困难问题上充分展开。
Self-Backtracking \cite{selfbacktracking2025}
则训练了一种更精细的控制能力：
模型学会在推理过程中插入 backtrack token，
主动回溯已发现有误的推理路径并从正确的分支重新开始——
这是一种\textit{错误恢复}机制，
而非简单的 early stopping 或 token budgeting。
结合方向十中 SelfBudgeter \cite{selfbudgeter2025}
和 AutoThink \cite{tu2025autothink} 的工作，
adaptive reasoning 的技术路线已覆盖三个层次：
\textit{是否推理}（AdaptThink）、
\textit{推理多深}（SelfBudgeter/AutoThink）、
\textit{推理出错怎么办}（Self-Backtracking）。

\noindent
未来的方向指向系统层面的整合：
reasoning as a metered resource ---
在 inference 服务层面将 ``thinking tokens'' 作为独立的计算资源进行定价和调度，
使得不同应用场景可以根据 latency-accuracy tradeoff 选择最优的推理预算。
这一方向连接了 Thread~4（reasoning 能力）和 Thread~7（efficiency），
是 post-training 技术从实验室走向大规模服务部署的关键桥梁。

\paragraph{方向七：From Scalar Metrics to Multi-Dimensional Capability Observability。}
...
\noindent
这一方向有三个子方向值得深入：
(1) \textbf{IRT-based 能力 profiling}——
ATLAS \cite{li2025atlas} 用自适应测试将评估 item 数减少 90\%；
(2) \textbf{动态/对抗性评估}——
LiveBench \cite{white2024livebench}、AutoDetect \cite{cheng2024autodetect}
将评估集成到训练循环中；
(3) \textbf{训练过程中的机制追踪}——
Wu et al.\ \cite{wu2024featuretracking} 使用 SAE
追踪 feature-level 的动态变化。

\paragraph{方向八：Diagnostic-Driven Adaptive Training --- ``Train What You Don't Know''。}
方向七解决了``如何观测能力''，方向八则问：
\textbf{观测到能力 profile 后，如何自适应地调整训练？}
核心原则是 \textit{closed loop}：
诊断弱点 $\rightarrow$ 生成针对性数据 $\rightarrow$ 训练 $\rightarrow$ 重新诊断。

DPE \cite{jia2026dpe} 是这一原则的典型实现
（技术细节见 \S\ref{sec:t5-self-evolving}）。
其 $p(1-p)$ 分析揭示了一个关键的信息论原理：
...
所有方法都收敛于同一原则：
\textbf{在模型决策边界附近采样以最大化信息增益}。

一个重要的实践问题是 difficulty labels 的时效性。
Zhang et al.\ \cite{zhang2025adcl} 报告了``Difficulty Shift''现象：
随着模型能力变化，静态的 difficulty 标签快速过时，
这进一步论证了 closed-loop diagnostic 的必要性。

\noindent
上述工作聚焦于 $p \approx 0.5$ 的决策边界区域，
但 \textbf{高置信度错误}（$p$ 接近 1 但实际错误的样本）
构成了一个互补的高价值训练信号来源。
ACE \cite{ace2026} 发现标准 RLVR 对所有错误施加均匀惩罚，
虽然提升了 pass@1 但收窄了模型的推理边界、降低了多样性。
ACE 提出 \textbf{asymmetric confidence penalty}——
对 overconfident errors 施加更强的惩罚，
迫使模型重新审视自己``确信正确但实际错误''的认知盲区。
LENS \cite{feng2025lens} 则解决了 GRPO 的一个关键低效问题：
当一组采样中没有正确答案时，
标准 GRPO 产生零梯度、浪费了这些 negative samples。
LENS 通过 confidence-weighted penalty
从全错样本组中提取训练信号，
使模型从自己的``彻底失败''中也能学到东西。
DPE 的 $p(1-p)$ 区域与 ACE/LENS 的高置信错误区域
共同描绘了一幅更完整的训练信号分布图：
\textbf{决策边界处提供最大信息增益，
高置信错误区域则矫正最危险的认知错觉}。

\paragraph{方向九：Breaking Static Data Saturation --- 持续自我进化。}
...
\noindent
解决 saturation 的方案可分为三类：
(1) \textbf{Corpus-grounded self-play}——
SPICE \cite{liu2025spice} 将 self-play 锚定在外部语料库上，
SPELL \cite{yang2025spell} 设计 three-role self-play 架构；
(2) \textbf{Meta-RL teacher}——
SOAR \cite{sundaram2026soar} 生成恰好位于学生当前能力边界之上的 stepping-stone problems；
(3) \textbf{Replay buffer engineering}——
ExGRPO \cite{zhan2025exgrpo} 引入结构化 replay buffer。

\paragraph{方向七至九的统一视角。}
三个方向构成了一个完整的自适应学习闭环：
...
\paragraph{方向十：Meta-Capability Training --- 教会模型``如何思考''。}
前述方向聚焦于训练\textit{过程}的优化。
方向十则追问：
...
\textbf{连接工具}（教模型使用外部工具，\crossref{6}{sec:thread6}）。
...
DeepSeek-R1 的``aha moment''揭示了 RL 可以涌现 self-reflection 行为。
...
其深层原因在于 SFT 倾向于记忆表面 pattern
（如特定的 CoT 模板或格式），
而 RL 通过 outcome-based reward
强化的是\textit{策略选择}本身而非策略的具体形式。
\textbf{SCoRe} \cite{kumar2024score}
首次证明 multi-turn RL 可以有效训练 self-correction，
并揭示了 SFT 训练 self-correction 失败的两个机制原因：
（1）distribution mismatch——SFT 训练数据反映的是数据收集策略的错误分布，
而非模型自身的错误分布；
（2）behavior collapse——模型在 SFT 下倾向于收敛到非纠正模式。
...
\noindent
\textbf{Meta-R1} \cite{dong2025metar1} 增加了显式的 meta-level cognitive layer，
\textbf{MERA} \cite{ha2025mera} 将推理与控制解耦以解决 overthinking。
在更广泛的维度上，
S$^2$R \cite{ma2025s2r} 训练 self-verify + self-correct，
SelfBudgeter \cite{selfbudgeter2025} 和 AutoThink \cite{tu2025autothink}
实现了 adaptive token allocation（50--60\% token 压缩而无 accuracy 损失）。

\noindent
\textbf{Calibration 作为最高杠杆的 meta-capability。}
在所有 meta-capability 中，calibration（模型对自身正确性的准确估计）
可能是杠杆最高的一个——
它既是 self-correction 的前提（不知道自己错了就无法纠正），
也是 effort allocation 的基础（不知道问题难度就无法合理分配计算资源），
还是 safety 的关键维度（overconfident 的错误答案比承认不确定更危险）。
然而，当前的 reasoning training 与 calibration 之间存在深刻张力：
AbstentionBench \cite{abstentionbench2025} 发现
reasoning fine-tuning 平均降低模型的 abstention 能力 24\%，
即使在 reasoning model 本身更擅长的领域也是如此——
\textbf{推理训练让模型变得更强但也更``不知道自己不知道什么''}。
Leng et al.\ \cite{leng2024tamingoverconfidence}
揭示了这一问题的根源：RLHF 中的 reward model 对高置信度回答存在系统性偏好，
无论回答质量如何，导致训练后的模型表达过度自信。

\noindent
针对 calibration 的 RL 训练正在形成一条新的技术路线。
RLCR \cite{damani2025rlcr} 提出使用 bounded proper scoring rule（如 Brier score）
作为 RL 的 reward function——
模型在生成答案的同时预测自身的置信度，
proper scoring rule 保证诚实报告是最优策略。
``Rewarding Doubt'' \cite{stangel2025rewardingdoubt}
基于 logarithmic scoring rule 设计 reward function，
同时惩罚 over-confidence 和 under-confidence，
在事实性问答等任务上显著改善了 calibration。
MASA \cite{masa2025} 则将 calibration 扩展为更一般的 meta-awareness：
通过 RL 训练模型对自身推理过程的预测准确性，
并证明这种 meta-aware 能力可以跨域迁移。

\noindent
关键理论问题是 meta-capability 是否跨领域迁移。
Li et al.\ \cite{transfer2025} 提供了正面证据——
RL-tuned 模型的推理改进跨领域泛化，而 SFT 不会。
Sun et al.\ \cite{sun2025grokking} 发现 RL 可触发 genuine phase transition，
类似于 supervised learning 中的 grokking 现象。
综合来看，meta-capability training 的未来方向是
...
\paragraph{方向十一：Multi-Objective Post-Training --- Pareto 优化。}
