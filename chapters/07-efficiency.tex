...
\paragraph{数据成本。}
高质量 instruction\hyp{}following 数据的人工标注成本高昂：
...
（视觉语言模型的 post-training 方法见 \crossref{5}{sec:t5}）。
...
LIMA 的实验细节和 hypothesis 的深入讨论见 Thread~1（\S\ref{sec:t1-quality}）。
...
（\crossref{2}{sec:t2-rlvr}）不仅在推理性能上取得突破，
...
% Branches
\node[branch, below left=1.2cm and 0.5cm of qlora] (brA) {Branch A: 合成数据\\Self-Instruct, Alpaca};
\node[branch, below right=1.2cm and 0.5cm of qlora] (brB) {Branch B: 数据策展\\phi-1, IFD, DEITA};
...
\node[branch, below=1.2cm of grpo] (conv) {收敛：数据 + 参数 + 算法\\三路效率融合};
...
\end{tikzpicture}
\caption{Thread 7 演化路径：从参数效率突破到数据--参数--算法三路融合。
棕色节点为 landmark 工作，灰色节点为重要分支。}
\label{fig:efficiency-evolution}
\end{figure}

\begin{table}[!ht]
\centering
\caption{Thread 7 核心方法对比。Efficiency Dimension 区分参数效率、数据效率和算法效率。}
...
这条线程与 Thread~1（数据策展的基础方法，\crossref{1}{sec:t1}）、
Thread~2（偏好优化算法的简化，\crossref{2}{sec:t2}）、
Thread~4（推理能力的蒸馏，\crossref{4}{sec:t4}）
存在深度交织。
