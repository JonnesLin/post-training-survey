...
Super-Natural\-Instructions——一个包含 1,616 个任务的大规模 benchmark，
...
Qwen3 \cite{yang2025qwen3} 的四阶段 post-training 流水线（详见 \crossref{4}{sec:t4-thinking-models}）
清晰地展示了 SFT 在现代训练范式中的多重角色：
SFT 同时出现在 Stage 1（long CoT cold start，推理格式初始化）
和 Stage 3（thinking mode fusion，推理/非推理模式融合）中，
标志着 SFT 从``一次性训练步骤''演变为``可在流水线中多次使用的通用对齐工具''。
...
进一步拓宽了 SFT 方法论的适用边界
（多模态 post-training 的系统讨论见 \crossref{5}{sec:t5}）。
