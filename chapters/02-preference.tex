...
从 RLHF 的基础框架（\S\ref{sec:t2-foundation}），
到其核心局限性催生的多条分支路径（\S\ref{sec:t2-branches}），
再到近期方法在效率与效果之间寻找新平衡点的收敛趋势（\S\ref{sec:t2-convergence}），
最终讨论仍未解决的 open problems（\S\ref{sec:t2-open}）。
...
人工标注的 (instruction, response) 对。这一范式虽然简洁有效（\crossref{1}{sec:t1}），
但存在两个根本性的局限。
...
这一设计直接预防了后来被广泛研究的 reward overoptimization 问题（\S\ref{sec:t2-reward-hacking}）。
...
这一思想后来深刻影响了 reasoning verification（\crossref{4}{sec:t4}）和
GRPO 中的 process supervision（\S\ref{sec:t2-grpo}）。
\end{solutionbox}
...
\begin{table}[!ht]
\centering
\caption{Thread 2 核心方法对比。Needs RM 指是否需要独立的 learned reward model。}
...
Preference optimization 的演化路径可以概括为：
从 Christiano et al.\ 的 comparison-based reward learning 出发，
