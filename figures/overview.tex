...
% --- Geometry ---
\def\laneheight{1.4}   % vertical spacing between lanes
\def\xmin{0}
...
\node[paper, fill=thread1] (flan)
    at (\yearx{2022}-1.0, \laneA*\laneheight) {FLAN};
\node[paper, fill=thread1] (instructgpt-sft)
    at (\yearx{2022}+0.8, \laneA*\laneheight) {InstructGPT-SFT};
\node[paper, fill=thread1] (selfinstruct)
    at (\yearx{2023}-0.2, \laneA*\laneheight) {Self-Instruct};
\node[paper, fill=thread1] (lima)
    at (\yearx{2023}+1.2, \laneA*\laneheight) {LIMA};
...
\node[paper, fill=thread3] (cai)
    at (\yearx{2022}+1.5, \laneC*\laneheight) {Constitutional AI};
\node[paper, fill=thread3] (saferlhf)
    at (\yearx{2023}+1.0, \laneC*\laneheight) {Safe RLHF};
\node[paper, fill=thread3] (jailbreak)
    at (\yearx{2024}+0.5, \laneC*\laneheight) {Jailbreak Arms Race};
...
\node[paper, fill=thread5] (llava)
    at (\yearx{2023}+0.5, \laneE*\laneheight) {LLaVA};
\node[paper, fill=thread5] (vlmrlhf)
    at (\yearx{2024}-0.3, \laneE*\laneheight) {VLM-RLHF};
\node[paper, fill=thread5] (halluc)
    at (\yearx{2024}+1.3, \laneE*\laneheight) {Halluc. Reduction};
...
\node[paper, fill=thread6] (toolformer)
    at (\yearx{2023}-0.2, \laneF*\laneheight) {Toolformer};
\node[paper, fill=thread6] (funccall)
    at (\yearx{2023}+1.5, \laneF*\laneheight) {Function Calling};
\node[paper, fill=thread6] (agenttune)
    at (\yearx{2024}+0.8, \laneF*\laneheight) {Agent Tuning};
\node[paper, fill=thread6] (mcp)
    at (\yearx{2025}+1.0, \laneF*\laneheight) {MCP/Agents};
...
\draw[crossarrow, thread1!70] (instructgpt-sft.south) -- (instructgpt-rlhf.north);

% T2 -> T3: RLHF ideas feed into Constitutional AI
\draw[crossarrow, thread2!70] (instructgpt-rlhf.south) -- (cai.north);

% T2 -> T4: Reward modeling feeds reasoning verification (PRM)
\draw[crossarrow, thread2!70] (dpo.south)
    to[out=-90, in=90] (prm.north);

% T4 -> T2: Reasoning verification inspires GRPO
\draw[crossarrow, thread4!70] (prm.east)
    to[out=0, in=180] (grpo.south west);

% T1 -> T5: SFT pipeline adopted by LLaVA
\draw[crossarrow, thread1!70] (selfinstruct.south)
